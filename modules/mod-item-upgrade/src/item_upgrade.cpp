...
void ItemUpgrade::MergeStatRequirements(std::unordered_map<uint32, StatRequirementContainer>& statRequirementMap, bool validate)
{
    for (auto& statPair : statRequirementMap)
    {
        ...
        if (honorTotal > 0.0f)
        {
            ...
            if (validate && (val < 1 || static_cast<uint32>(val) > sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS)))
        ...
        if (arenaTotal > 0.0f)
        {
            ...
            if (validate && (val < 1 || static_cast<uint32>(val) > sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS)))
...
bool ItemUpgrade::ValidateReq(uint32 id, UpgradeStatReqType reqType, float val1, float val2, const std::string& table) const
{
    ...
    switch (reqType)
    {
        ...
        case ItemUpgrade::REQ_TYPE_HONOR:
            if (val1Int >= 1 && static_cast<uint32>(val1Int) <= sWorld->getIntConfig(CONFIG_MAX_HONOR_POINTS))
                return true;
            LOG_ERROR("sql.sql", "Table `{}` has invalid `req_val1` {} (honor points) for `id` {}, skip", table, val1, id);
            return false;
        case ItemUpgrade::REQ_TYPE_ARENA:
            if (val1Int >= 1 && static_cast<uint32>(val1Int) <= sWorld->getIntConfig(CONFIG_MAX_ARENA_POINTS))
                return true;
...
{
    return ItemIcon(proto, 20, 20, 0, 0);
}
...
bool ItemUpgrade::_AddPagedData(Player* player, const PagedData& pagedData, uint32 page) const
{
        ...
        || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERCS || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERC_INFO || pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK_INFO)
    {
        ...
        if (pagedData.type == PAGED_DATA_TYPE_STATS)
        {
            ...
            ossStatTypes << "TIENE ESTADÍSTICAS: ";
        ...
        else if (pagedData.type == PAGED_DATA_TYPE_REQS)
        {
            ...
            oss << "REFORJAR " << StatTypeToString(upgradeStat->statType) << " [RANGO " << upgradeStat->statRank << "]";
            oss << " " << "[" << upgradeStat->statModPct << "% incrementa - ";
        ...
        else if (pagedData.type == PAGED_DATA_TYPE_UPGRADED_ITEMS_STATS)
        {
            ...
            std::ostringstream oss;
            oss << "El nivel de objeto aumentó en " << diff;
            ...
            if (weaponUpgrade != nullptr)
            {
                ...
                wuoss << "|cff056e3aDAÑO DE ARMA REFORJADO EN " << FormatFloat(weaponUpgrade->statModPct) << "%|r";
            ...
            {
                std::ostringstream wuoss;
                wuoss << "|cff056e3aVELOCIDAD DE ARMA REFORJADA EN " << FormatFloat(weaponSpeedUpgrade->statModPct) << "%|r";
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, wuoss.str(), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
            }

            if (!item->IsEquipped())
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "[EQUIPAR OBJETO]", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1);
        }
        else if (pagedData.type == PAGED_DATA_TYPE_STAT_UPGRADE_BULK)
        {
            ...
            std::ostringstream oss;
            if (upgrades.empty())
                oss << "[NIVEL OBJETO |cffb50505won't|r aumentar, sin reforjas para aplicar]";
            else
            {
                ...
                oss << "[NIVEL OBJETO |cffb50505" << ilvl.first << "|r --> " << "|cff056e3a" << ilvl.second << "|r]";
                oss << " [ACTUAL: " << currentIlvl.second << "]";
            }
    ...
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Total items reforjados: " + Acore::ToString(pagedData.data.size()), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Total reforjas: " + Acore::ToString(totalUpgrades), GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
    }
    else if (pagedData.type == PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505PURGAR REFORJAS|r", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        const ItemTemplate* proto = sObjectMgr->GetItemTemplate((uint32)GetIntConfig(CONFIG_ITEM_UPGRADE_PURGE_TOKEN));
        if (proto != nullptr)
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Recibirá después de purgar:", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        ...
        if (GetBoolConfig(CONFIG_ITEM_UPGRADE_REFUND_ALL_ON_PURGE))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cff056e3aSE REEMBOLSARÁ TODO EN LA PURGA|r", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Elija un objeto reforjado para purgar:", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page);
    }
    ...
    for (uint32 i = lowIndex; i <= highIndex; i++)
    {
        ...
        if (pagedData.type != PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
            ...
            AddGossipItemFor(player, identifier->optionIcon, identifier->uiName, GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + identifier->id, "¿Estás seguro de que deseas purgar todas las reforjas? ¡Esto no se puede deshacer!", 0, false);
    }

    if (pagedData.type == PAGED_DATA_TYPE_REQS)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsRequirement(player, pagedData.upgradeStat, item) ? "|cff19ff00[COMPRAR]|r" : "|cffFF0000[FALTAN REQUISITOS]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "¿Estás seguro de que deseas reforjar?", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERC_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsWeaponUpgradeRequirement(player) ? "|cff056e3a[REFORJAR]|r" : "|cffb50505[REFORJAR]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "¿Estás seguro de que deseas reforjar esta arma?", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_PERC_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsWeaponSpeedUpgradeRequirement(player) ? "|cff056e3a[REFORJAR]|r" : "|cffb50505[REFORJAR]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "¿Estás seguro de que deseas reforjar la velocidad de esta arma?", 0, false);

    if (!upgrades.empty())
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[TOTAL REQUISITOS]", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1);
        StatRequirementContainer reqs = BuildBulkRequirements(upgrades, item);
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, (MeetsRequirement(player, &reqs) ? "|cff056e3a[REFORJAR TODO]|r" : "|cffb50505[REFORJAR TODO]|r"), GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 2, "¿Estás seguro de que deseas reforjar?", 0, false);
    }

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS_CHECK_INFO)
        // AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cffb50505REMOVER REFROJA|r", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Are you sure you want to remove this weapon upgrade? This cannot be undone!", 0, false);
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cffb50505REMOVER REFROJA|r", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Estás seguro de que deseas eliminar esta reforja de arma? ¡Esto no se puede deshacer!", 0, false);

    if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK_INFO)
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "|cffb50505REMOVER REFORJA DE VELOCIDAD|r", GOSSIP_SENDER_MAIN + 1, GOSSIP_ACTION_INFO_DEF + 1, "Estás seguro de que deseas eliminar esta reforja de velocidad de arma? ¡Esto no se puede deshacer!", 0, false);

    if (page + 1 < pagedData.totalPages)
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "[Siguiente] ->", GOSSIP_SENDER_MAIN + 2, GOSSIP_ACTION_INFO_DEF + page + 1);
    ...
    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Atras]", page == 0 ? pageZeroSender : GOSSIP_SENDER_MAIN + 2, page == 0 ? GOSSIP_ACTION_INFO_DEF : GOSSIP_ACTION_INFO_DEF + page - 1);
...
void ItemUpgrade::NoPagedData(Player* player, const PagedData& pagedData) const
{
    if (pagedData.type == PAGED_DATA_TYPE_ITEMS || pagedData.type == PAGED_DATA_TYPE_UPGRADED_ITEMS || pagedData.type == PAGED_DATA_TYPE_ITEMS_BULK || pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505NADA EN ESTA PAGINA|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        ...
        || pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERCS || pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERC_INFO)
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505EL ITEM NO PUEDE SER REFORJADO|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    }
    else if (pagedData.type == PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505NADA PARA PURGAR|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_ITEMS)
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505NADA EN ESTA PAGINA, SOLO SE MOSTRARAN LOS ITEMS EQUIPADOS|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Primera página]", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
}
...
bool ItemUpgrade::TakePagedDataAction(Player* player, Creature* creature, uint32 action)
{
    ...
    if (pagedData.type == PAGED_DATA_TYPE_ITEMS)
    {
        ...
        if (item == nullptr)
            SendMessage(player, "El objeto ya no está disponible para la reforja.");
        else
        {
        ...
        if (identifier != nullptr)
        {
            ...
            else
            {
                ...
                if (upgradeStat == nullptr)
                    SendMessage(player, "reforja ya no disponible.");
                else
                {
                    ...
                    if (playerUpgrade != nullptr)
                    {
                        ...
                        {
                            SendMessage(player, "Ya está en el rango máximo para esta estadística y objeto.");
                            BuildStatsUpgradeCatalogue(player, item);
                    ...
                    {
                        SendMessage(player, "Este rango no está disponible para " + ItemLink(player, item));
                        BuildStatsUpgradeCatalogue(player, item);
                        return AddPagedData(player, creature, pagedData.currentPage);
                    }
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_REQS)
    {
        ...
        else
        {
            if (!PurchaseUpgrade(player))
                SendMessage(player, "La reforja no se pudo procesar. Esto no debería suceder, a menos que el objeto ya no esté disponible.");
            else
            {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_ITEMS_FOR_PURGE)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "El objeto ya no está disponible.");
        else
        {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "El arma ya no está disponible para la reforja.");
        else
        {
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_UPGRADE_PERCS)
    {
        ...
        if (identifier != nullptr && identifier->GetType() == FLOAT_IDENTIFIER)
        {
            ...
            if (!IsValidWeaponForUpgrade(item, player))
                SendMessage(player, "El arma ya no está disponible.");
            else
            {
                auto rebuildPage = [&]()
                {
                ...
                if (weaponUpgrade != nullptr)
                {
                    ...
                    {
                        SendMessage(player, "¡Ya compraste esta reforja de arma!");
                        return rebuildPage();
                    }
                    else
                    {
                        ...
                        if (nextWeaponUpgrade != nullptr)
                        {
                            ...
                            {
                                SendMessage(player, "¡Debes comprar la reforja anterior primero!");
                                return rebuildPage();
                            }
                            else
                            {
        ...
        else
        {
            ...
            else if (action == 1)
            {
                if (!PurchaseWeaponUpgrade(player))
                    SendMessage(player, "La reforja no se pudo procesar. Esto no debería suceder, a menos que el arma ya no esté disponible.");
                else
                {
                    CloseGossipMenuFor(player);
                    return true;
    ...
    else if (pagedData.type == PAGED_DATA_TYPE_WEAPON_SPEED_ITEMS)
    {
        Item* item = FindItemIdentifierFromPage(pagedData, action, player);
        if (item == nullptr)
            SendMessage(player, "El arma ya no está disponible para la reforja de velocidad.");
        else
        {
        ...
        if (identifier != nullptr && identifier->GetType() == FLOAT_IDENTIFIER)
        {
            ...
            else
            {
                ...
                if (weaponUpgrade != nullptr)
                {
                    if (weaponUpgrade->statModPct >= floatIdentifier->modPct)
                    {
                        SendMessage(player, "¡Ya compraste esta reforja de velocidad de arma!");
                        return rebuildPage();
                    }
                    else
                    {
...
bool ItemUpgrade::PurchaseUpgrade(Player* player)
{
    ...
    if (!MeetsRequirement(player, pagedData.upgradeStat, item))
    {
        ...
        SendMessage(player, "Tu no cumples con los requisitos para comprar esta reforja.");
        return true;
    }
    ...
    VisualFeedback(player);
    SendMessage(player, "¡Objeto reforjado con éxito!");
...
bool ItemUpgrade::PurchaseWeaponUpgrade(Player* player)
{
    ...
    VisualFeedback(player);
    SendMessage(player, "¡Arma reforjada con éxito!");
...
bool ItemUpgrade::PurchaseWeaponSpeedUpgrade(Player* player)
{
    ...
    {
        SendMessage(player, "Tu no cumples con los requisitos para comprar esta reforja de velocidad.");
        return true;
    }
    ...
    VisualFeedback(player);
    SendMessage(player, "¡Velocidad de arma reforjada con éxito!");
...
bool ItemUpgrade::PurchaseUpgradeBulk(Player* player)
{
    ...
    {
        SendMessage(player, "Tu no cumples con los requisitos para comprar estas reforjas.");
        return true;
    }
...
void ItemUpgrade::BuildRequirementsPage(const Player* player, PagedData& pagedData, const StatRequirementContainer* reqs) const
{
    if (EmptyRequirements(reqs))
    {
        ...
        identifier->id = 0;
        identifier->name = "0";
        identifier->uiName = "SIN REQUISITOS, SE PUEDE COMPRAR LIBREMENTE";
        ...
        for (const auto& req : *reqs)
        {
            ...
            switch (req.reqType)
            {
                case REQ_TYPE_COPPER:
                    oss << "ORO: " << CopperToMoneyStr((uint32)req.reqVal1, true);
                    break;
                case REQ_TYPE_HONOR:
            ...
            if (!MeetsRequirement(player, req))
            {
                ...
                {
                    case REQ_TYPE_COPPER:
                        missing = "faltante " + CopperToMoneyStr((uint32)req.reqVal1 - player->GetMoney(), true);
                        break;
                    case REQ_TYPE_HONOR:
                        missing = "faltante " + Acore::ToString<uint32>((uint32)req.reqVal1 - player->GetHonorPoints()) + " puntos";
                        break;
                    case REQ_TYPE_ARENA:
                        missing = "faltante " + Acore::ToString<uint32>((uint32)req.reqVal1 - player->GetArenaPoints()) + " puntos";
                        break;
                    case REQ_TYPE_ITEM:
                        missing = "faltante " + Acore::ToString<uint32>((uint32)req.reqVal2 - player->GetItemCount((uint32)req.reqVal1, true)) + " items";
                        break;
            ...
            oss << " - ";
            if (missing.empty())
                oss << "|cff056e3aCOMPLETADO|r";
            else
                oss << "|cffb50505EN PROGRESO|r" << " - " << missing;
    ...
    if (shouldAdd)
    {
        ...
        if (pagedData.type != PAGED_DATA_TYPE_WEAPON_UPGRADE_ITEMS_CHECK && pagedData.type != PAGED_DATA_TYPE_WEAPON_SPEED_UPGRADE_ITEMS_CHECK)
        {
            if (!IsAllowedItem(item) || IsBlacklistedItem(item))
                itemIdentifier->uiName += " [|cffb50505INACTIVO|r]";
        }
...
void ItemUpgrade::BuildItemUpgradeStatsCatalogue(const Player* player, const Item* item)
{
    ...
    if (!itemUpgrades.empty())
    {
        ...
        for (const UpgradeStat* upgradeStat : itemUpgrades)
        {
            ...
            std::ostringstream oss;
            oss << "REFORJADO " << statTypeStr << " [RANGO " << upgradeStat->statRank << "]";
            oss << " " << "[" << upgradeStat->statModPct << "% incrementa - ";
...
void ItemUpgrade::_BuildWeaponPercentUpgradesCatalogue(const Player* player, const Item* item, PagedDataType type)
{
    ...
    for (size_t i = 0; i < upgradeStats.size(); i++)
    {
        ...
        if (toPurchase)
            ...
            oss << "|cffb50505";
        oss << "Reforjar en " << identifier->modPct << "%|r";
        if (toPurchase)
            oss << " [COMPRAR]";
        else if (purchased)
            oss << " [HECHO]";
...
void ItemUpgrade::BuildWeaponUpgradesPercentInfoCatalogue(const Player* player, const Item* item, float pct)
{
    ...
    pctIdnt->id = 0;
    pctIdnt->uiName = "Reforjando el daño en " + FormatFloat(pct) + "%";
    pagedData.data.push_back(pctIdnt);

    Identifier* identifier = new Identifier();
    identifier->id = 0;
    identifier->uiName = "Requisitos:";
    pagedData.data.push_back(identifier);
    ...
    minDmgIdnt->id = 0;
    minDmgIdnt->uiName = "DAÑO MÍNIMO " + FormatIncrease(currentMinDamage, nextMinDamage);
    pagedData.data.push_back(minDmgIdnt);

    Identifier* maxDmgIdnt = new Identifier();
    maxDmgIdnt->id = 0;
    maxDmgIdnt->uiName = "DAÑO MÁXIMO " + FormatIncrease(currentMaxDamage, nextMaxDamage);
...
void ItemUpgrade::BuildWeaponSpeedUpgradesPercentInfoCatalogue(const Player* player, const Item* item, float pct)
{
    ...
    Identifier* pctIdnt = new Identifier();
    pctIdnt->id = 0;
    pctIdnt->uiName = "Reforjando la velocidad en " + FormatFloat(pct) + "% (reducirá el retraso de golpe del arma)";
    pagedData.data.push_back(pctIdnt);
    ...
    origDelayIdnt->id = 0;
    origDelayIdnt->uiName = "VELOCIDAD ORIGINAL: " + FormatDelay(originalDelay);
    ...
    currentDelayIdnt->uiName = "VELOCIDAD ACTUAL: |cffb50505" + FormatDelay(currentDelay) + "|r";
    ...
    nextDelayIdnt->id = 0;
    nextDelayIdnt->uiName = "SE REDUCIRÁ A: |cff056e3a" + FormatDelay(nextDelay) + "|r";
...
void ItemUpgrade::BuildWeaponUpgradeInfoCatalogue(const Player* player, const Item* item)
{
    ...
    idnt->id = 0;
    idnt->name = "0";
    idnt->uiName = "Daño reforjado en " + FormatFloat(weaponUpgrade->statModPct) + "%";
    ...
    minDmgIdnt->id = 0;
    minDmgIdnt->name = "1";
    minDmgIdnt->uiName = "DAÑO MÍNIMO " + FormatIncrease(dmgInfo.first, upgradedDmgInfo.first);
    pagedData.data.push_back(minDmgIdnt);

    Identifier* maxDmgIdnt = new Identifier();
    maxDmgIdnt->id = 0;
    maxDmgIdnt->name = "2";
    maxDmgIdnt->uiName = "DAÑO MÁXIMO " + FormatIncrease(dmgInfo.second, upgradedDmgInfo.second);
    pagedData.data.push_back(maxDmgIdnt);
    ...
    if (!item->IsEquipped())
    {
        ...
        equipIdnt->id = 2;
        equipIdnt->name = "3";
        equipIdnt->uiName = "[EQUIPAR OBJETO]";
...
void ItemUpgrade::BuildWeaponSpeedUpgradeInfoCatalogue(const Player* player, const Item* item)
{
    ...
    speedIdnt->id = 0;
    speedIdnt->name = "0";
    speedIdnt->uiName = "Velocidad reforjada en " + FormatFloat(weaponUpgrade->statModPct) + "%";
    ...
    newDelayIdnt->id = 0;
    newDelayIdnt->name = "2";
    newDelayIdnt->uiName = "VELOCIDAD ACTUAL: " + FormatDelay(newDelay);
...
void ItemUpgrade::BuildStatsUpgradeCatalogue(const Player* player, const Item* item)
{
    ...
    if (IsAllowedItem(item) && !IsBlacklistedItem(item))
    {
        ...
        for (const UpgradeStat& stat : upgradeStatList)
        {
            ...
            Identifier* identifier = new Identifier();
            std::ostringstream oss;
            oss << "REFORJAR " << StatTypeToString(statInfo->ItemStatType) << " ";
            if (foundUpgrade != nullptr)
            {
                ...
                {
                    oss << "[RANGO " << foundUpgrade->statRank << " |cffb50505MÁX|r]";
                    identifier->id = foundUpgrade->statId;
                    atMaxRank = true;
                }
                else
                {
                    oss << "[RANGO " << foundUpgrade->statRank << " -> " << "|cff056e3a" << foundUpgrade->statRank + 1 << "|r" << "]";
            ...
            else
            {
                ...
                oss << "[ADQUIRIR RANGO 1]";
                identifier->id = foundUpgrade->statId;
            }

            oss << " " << "[" << foundUpgrade->statModPct << "% incrementa - ";
            oss << "|cffb50505" << statInfo->ItemStatValue << "|r --> ";
            oss << "|cff056e3a" << CalculateModPct(statInfo->ItemStatValue, foundUpgrade) << "|r]";
            if (currentUpgrade != nullptr)
            {
                oss << " [ACTUAL: " << CalculateModPct(statInfo->ItemStatValue, currentUpgrade);
            ...
            if (!atMaxRank && !CanApplyUpgradeForItem(item, foundUpgrade))
                oss << " [|cffb50505REFORJA PROHIBIDA|r]";
...
void ItemUpgrade::BuildStatsUpgradeCatalogueBulk(const Player* player, const Item* item)
{
    ...
    if (IsAllowedItem(item) && !IsBlacklistedItem(item))
    {
        for (const auto& upair : upgradesPctMap)
        {
            ...
            identifier->uiName = "Reforjar TODAS las estadísticas en " + FormatFloat(upair.first) + "%";
            pagedData.data.push_back(identifier);
        }
    }
...
void ItemUpgrade::BuildStatsUpgradeByPctCatalogueBulk(const Player* player, const Item* item, float pct)
{
    ...
    if (upgradesPctMap.find(pct) != upgradesPctMap.end())
    {
        ...
        for (const UpgradeStat* stat : upgrades)
        {
            ...
            if (!IsAllowedStatType(stat->statType))
                oss << "|cffb50505No|r se reforjará " << statTypeStr << ": estadística no permitida";
            else if (!CanApplyUpgradeForItem(item, stat))
                oss << "|cffb50505No|r se reforjará " << statTypeStr << ": rango no permitido para este objeto";
            else
            {
                ...
                if (currentUpgrade != nullptr)
                {
                    ...
                    if (nextUpgrade == nullptr)
                        oss << "|cffb50505No|r se reforjará " << statTypeStr << ": ya está en rango máximo";
                    else
                    {
                        if (currentUpgrade->statRank == stat->statRank - 1)
                            willUpgrade = true;
                        else if (currentUpgrade->statRank >= stat->statRank)
                            oss << "|cffb50505No|r se reforjará " << statTypeStr << ": rango ya adquirido";
                        else
                            oss << "|cffb50505No|r se reforjará " << statTypeStr << ": necesitas adquirir rangos anteriores";
                    }
                }
                else
                {
                ...
                if (willUpgrade)
                {
                    oss << "|cff056e3aSe|r reforjará " << statTypeStr << " a rango " << stat->statRank;
                    oss << " [" << FormatFloat(stat->statModPct) << "% incrementa, ";
                    oss << "|cffb50505" << foundStat->ItemStatValue << "|r --> ";
                    oss << "|cff056e3a" << CalculateModPct(foundStat->ItemStatValue, stat) << "|r]";

                    if (currentUpgrade != nullptr)
                        oss << " [ACTUAL: " << CalculateModPct(foundStat->ItemStatValue, currentUpgrade) << "]";
                }
            }
...
const ItemUpgrade::UpgradeStat* ItemUpgrade::FindNearestWeaponUpgradeStat(const UpgradeStatContainer& upgradeStatContainer, float pct) const
{
    ...
    for (size_t i = upgradeStatContainer.size(); i-- > 0;)
        if (upgradeStatContainer[i].statModPct < pct)
            return &upgradeStatContainer[i];

    for (size_t i = 0; i < upgradeStatContainer.size(); i++)
        if (upgradeStatContainer[i].statModPct > pct)
            return &upgradeStatContainer[i];
...
bool ItemUpgrade::TryAddItem(Player* player, uint32 entry, uint32 count, bool add)
{
    ...
    if (proto != nullptr)
    {
        ...
        if (msg != EQUIP_ERR_OK)
        {
            ...
            oss << " falló, revisa el espacio en tu inventario y vuelve a intentarlo.";
    ...
    for (const UpgradeStatReq& r : reqs)
    {
        switch (r.reqType)
        {
            case REQ_TYPE_COPPER:
                ...
                {
                    SendMessage(player, "No se puede reembolsar el oro/cobre: superarías el límite máximo de oro.");
                    return false;
                }
                break;
            case REQ_TYPE_ITEM:
...
bool ItemUpgrade::AddUpgradeForNewItem(Player* player, Item* item, const UpgradeStat* upgrade, const _ItemStat* stat)
{
    ...
    std::ostringstream oss;
    oss << "|cffeb891a[SISTEMA DE REFORJA]:|r";
    oss << " " << ItemLink(player, item);
    oss << " reforjó " << StatTypeToString(upgrade->statType) << " a RANGO " << upgrade->statRank << ".";
    oss << " Incrementa en " << upgrade->statModPct << "% [";
    oss << stat->ItemStatValue << " --> " << CalculateModPct(stat->ItemStatValue, upgrade) << "]";
    std::pair<uint32, uint32> itemLevel = CalculateItemLevel(player, item);
    oss << " [Nuevo ILVL: " << itemLevel.second << "]";
    SendMessage(player, oss.str());
...
/*static*/ std::string ItemUpgrade::FormatItemLocation(const Player* player, const Item* item)
{
    ...
    if (bagSlot == INVENTORY_SLOT_BAG_0)
    {
        ...
        if (itemSlot >= INVENTORY_SLOT_ITEM_START && itemSlot < INVENTORY_SLOT_ITEM_END)
            return "en mochila - slot " + Acore::ToString(itemSlot - INVENTORY_SLOT_ITEM_START + 1);
        if (itemSlot >= BANK_SLOT_ITEM_START && itemSlot < BANK_SLOT_ITEM_END)
            return "en banco - slot " + Acore::ToString(itemSlot - BANK_SLOT_ITEM_START + 1);
    }
    else
    {
        if (bagSlot >= INVENTORY_SLOT_BAG_START && bagSlot < INVENTORY_SLOT_BAG_END)
            return "en bolsa - " + Acore::ToString(bagSlot - INVENTORY_SLOT_BAG_START + 1) + " - slot " + Acore::ToString(itemSlot + 1);
        if (bagSlot >= BANK_SLOT_BAG_START && bagSlot < BANK_SLOT_BAG_END)
            return "en banco - bolsa " + Acore::ToString(bagSlot - BANK_SLOT_BAG_START + 1) + " - slot " + Acore::ToString(itemSlot + 1);
    }
