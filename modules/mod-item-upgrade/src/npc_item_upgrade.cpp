...
private:
    ...
    static Item* GetPagedDataItem(const ItemUpgrade::PagedData& pagedData, Player* player)
    {
        ...
        if (!validItem)
        {
            ItemUpgrade::SendMessage(player, "El objeto ya no está disponible para reforja.");
            return nullptr;
        }
    ...
    bool AddUpgradeWeaponsSubmenu(Player* player, Creature* creature)
    {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Selecciona un arma para reforjar", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 9);
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Ver armas reforjadas", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 10);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Volver]", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
    ...
    {
        ClearGossipMenuFor(player);
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Selecciona un arma para reforjar su velocidad", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 12);
        AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Ver armas reforjadas", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 13);
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- [Volver]", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        return true;
    }
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            ItemUpgrade::SendMessage(player, "El sistema de Reforja de Objetos está siendo recargado por un administrador, por favor inténtelo de nuevo más tarde.");
        ...
        if (!sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_ENABLED))
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "|cffb50505NO DISPONIBLE|r", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        else
        {
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Selecciona un objeto para reforjar", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            // AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Selecciona un objeto para reforjar (todas las estadísticas a la vez)", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 7);
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_ALLOW_PURGE))
                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Purgar mejoras", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 6);
            AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Ver objetos reforjados", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_WEAPON_DAMAGE))
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "[Sistema de reforja de daño de armas] ->", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 8);
            if (sItemUpgrade->GetBoolConfig(CONFIG_ITEM_UPGRADE_WEAPON_SPEED))
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "[Sistema de reforja de velocidad de armas] ->", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 11);
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Actualizar caché visual", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);

            if (player->GetSession()->GetSecurity() == SEC_ADMINISTRATOR)
                AddGossipItemFor(player, GOSSIP_ICON_INTERACT_1, "Bloquear para edición de base de datos", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5, "El NPC ya no estará disponible para los jugadores hasta que liberes el bloqueo con el comando .item_upgrade reload.", 0, false);
        }
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Salir...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        return true;
    }
