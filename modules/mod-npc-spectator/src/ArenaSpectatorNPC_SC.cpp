...
public:
    ...
    bool OnGossipHello(Player *player, Creature* creature) override {
        ...
        AddGossipItemFor(player, GOSSIP_ICON_DOT, "|TInterface\\icons\\achievement_boss_cthun:30:30:-20:0|tEspectar a jugador especifico", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_SPECIFIC, "", 0, true);
    ...
    bool OnGossipSelectCode(Player* player, Creature* creature, uint32 sender, uint32 action, const char* code) override {
        ...
        if (action == NPC_SPECTATOR_ACTION_SPECIFIC) {
            ...
            ChatHandler(player->GetSession()).PSendSysMessage("El jugador no está en línea o no existe.");
            OnGossipHello(player, creature);
            return false;
        }
        CloseGossipMenuFor(player);
