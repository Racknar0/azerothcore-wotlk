...
bool npc_1v1arena::OnGossipHello(Player* player, Creature* creature)
{
    ...
    if (player->InBattlegroundQueueForBattlegroundQueueType(bgQueueTypeId))
        ...
        // Traducir
        AddGossipItemFor(player, GOSSIP_ICON_DOT, "|TInterface/ICONS/Achievement_Arena_2v2_7:30:30:-20:0|t Salir de la cola de la arena 1v1", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_LEAVE_QUEUE, "¿Estás seguro?", 0, false);
    else
        ...
        // Traducir
        AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface\\icons\\Achievement_Arena_2v2_4:30:30:-20:0|t Unirse a la cola de la arena 1v1 (No Clasificada)", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_UNRATED);
    ...
    if (!teamExistForPlayerGuid(player))
    {
        ...
        // Traducir
        AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface/ICONS/Achievement_Arena_2v2_7:30:30:-20:0|t Crear nuevo equipo de arena 1v1", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_CREATE_ARENA_TEAM, "¿Estás seguro?", sConfigMgr->GetOption<uint32>("Arena1v1.Costs", 400000), false);
    }
    else
    {
        if (!player->InBattlegroundQueueForBattlegroundQueueType(bgQueueTypeId))
        {
            ...
            // Traducir
            AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface\\icons\\Achievement_Arena_2v2_1:30:30:-20:0|t Unirse a la cola de la arena 1v1 (Clasificada)", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_RATED);
            ...
            // Traducir
            AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface/ICONS/Achievement_Arena_2v2_7:30:30:-20:0|t Borrar equipo de arena", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_DISBAND_ARENA_TEAM, "¿Estás seguro?", 0, false);
        }
        ...
        // Traducir
        AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface/ICONS/INV_Misc_Coin_01:30:30:-20:0|t Mostrar tus estadísticas", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_GET_STATISTICS);
    }
    ...
    // Traducir
    AddGossipItemFor(player, GOSSIP_ICON_VENDOR, "|TInterface/ICONS/inv_misc_questionmark:30:30:-20:0|t Ayuda", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_ACTION_HELP);
    ...
    switch (action)
    {
        case NPC_ARENA_1V1_ACTION_CREATE_ARENA_TEAM:
        {
            ...
            else
            {
                ...
                handler.PSendSysMessage("Tienes que ser nivel {} + para crear un equipo de arena 1v1.", sConfigMgr->GetOption<uint32>("Arena1v1.MinLevel", 70));
                return true;
            }
        ...
        case NPC_ARENA_1V1_ACTION_JOIN_QUEUE_ARENA_RATED:
        {
            if (Arena1v1CheckTalents(player) && !JoinQueueArena(player, creature, true))
                ...
                // Traducir
                handler.SendSysMessage("Algo salió mal al unirte a la cola.");
        ...
        case NPC_ARENA_1V1_ACTION_GET_STATISTICS:
        {
            ...
            if (at)
            {
                ...
                // Traducir
                std::stringstream s;
                s << "\nClasificación: " << at->GetStats().Rating;
                s << "\nRango: " << at->GetStats().Rank;
                s << "\nPartidas de temporada: " << at->GetStats().SeasonGames;
                s << "\nVictorias de temporada: " << at->GetStats().SeasonWins;
        ...
        case NPC_ARENA_1V1_ACTION_DISBAND_ARENA_TEAM:
        {
            ...
            // Traducir
            handler.SendSysMessage("¡Equipo de arena eliminado!");
            CloseGossipMenuFor(player);
        ...
        case NPC_ARENA_1V1_ACTION_HELP:
        {
            ...
            // Traducir
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, "<- Volver", GOSSIP_SENDER_MAIN, NPC_ARENA_1V1_MAIN_MENU);
...
bool npc_1v1arena::JoinQueueArena(Player* player, Creature* /* me */, bool isRated)
{
    ...
    // Verificar si ya hay un jugador con la misma IP en la cola
    std::string playerIP = player->GetSession()->GetRemoteAddress();
    BattlegroundQueue& bgQueue = sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);

    // Iterar sobre todos los jugadores en la cola de batalla
    for (auto const& queuedPair : bgQueue.m_QueuedPlayers)
    {
        Player* queuedPlayer = ObjectAccessor::FindPlayer(queuedPair.first);
        // Añadimos una comprobación para no comparar al jugador consigo mismo
        if (queuedPlayer && queuedPlayer->GetGUID() != player->GetGUID() && queuedPlayer->GetSession()->GetRemoteAddress() == playerIP)
        {
            ChatHandler(player->GetSession()).SendSysMessage("No puedes unirte a la arena con jugadores que esten bajo la misma network que tú.");
            return false;
        }
    }
...
bool npc_1v1arena::CreateArenateam(Player* player, Creature* /* me */)
{
    ...
    // Traducir
    ChatHandler(player->GetSession()).SendSysMessage("¡Equipo de arena 1v1 creado con éxito!");
...
bool npc_1v1arena::Arena1v1CheckTalents(Player* player)
{
    ...
    if (player->HasHealSpec() && (sConfigMgr->GetOption<bool>("Arena1v1.PreventHealingTalents", false)))
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage("No puedes unirte porque tienes talentos prohibidos (Sanación)");
        return false;
    }
    ...
    if (player->HasTankSpec() && (sConfigMgr->GetOption<bool>("Arena1v1.PreventTankTalents", false)))
    {
        ...
        ChatHandler(player->GetSession()).SendSysMessage("No puedes unirte porque tienes talentos prohibidos (Tanque)");
        return false;
    }
