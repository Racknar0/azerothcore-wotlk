...
public:
    ...
    static bool HandleQueueArena1v1(ChatHandler* handler, const char* /*args*/, bool isRated)
    {
        ...
        if (!sConfigMgr->GetOption<bool>("Arena1v1.Enable", true))
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("Las arenas 1vs1 están desactivadas.");
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("No puedes estar en combate.");
            return false;
        }
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("Necesitas nivel {}+ para unirte a la arena en solitario.", minLevel);
            return false;
        }
        ...
        if (!player->GetArenaTeamId(sConfigMgr->GetOption<uint32>("Arena1v1.ArenaSlotID", 3)) && isRated)
        {
            ...
            handler->PSendSysMessage("¡Únete de nuevo a la arena 1v1 clasificada!");
        }
        else
        {
            if (Command1v1.JoinQueueArena(player, nullptr, isRated))
                handler->PSendSysMessage("Te has unido a la cola de la arena 1v1 en solitario {}.", isRated ? "Clasificada" : "No Clasificada");
        }
